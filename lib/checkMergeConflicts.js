...
const updateAllPRsLabel = 'PR: require post-merge sync to HEAD';
function sleep(ms) {
...
/**
 * Ping all pull requests to merge from develop if a PR that requires it got merged.
 * @param {import('probot').Context} context
 */
module.exports.pingAllPullRequestsToMergeFromDevelop = async function (
  ...
  if (prLabels.includes(updateAllPRsLabel)) {
    ...
    const allPullReqests = pullRequestsResponse.data;
    allPullReqests.forEach(async (pullRequest) => {
