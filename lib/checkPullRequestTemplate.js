...
/**
 * This function checks if head and base of the PR are from the same
 * repository.
 *
 */
const compareHeadAndBase = (prHead, prBase) => {
  console.log(prHead, prBase);
  var reRepoName = /^(.+?):/gm;
  var prHeadRepo = reRepoName.exec(prHead);
  reRepoName.lastIndex = 0;
  var prBaseRepo = reRepoName.exec(prBase);
  console.log(prHeadRepo[1], prBaseRepo[1]);
  return prHeadRepo[1] === prBaseRepo[1];
};
/**
    ...
    uncheckedItems.forEach((item) => {
      ...
      if (item.toLowerCase().includes(EDITS_FROM_MAINTAINERS_TEXT)) {
        ...
        if (!pullRequest.maintainer_can_modify &&
          !compareHeadAndBase(pullRequest.head.label,
