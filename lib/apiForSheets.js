...
module.exports.generateOutput = async function (
    rows, prNumber, prDetails, repositoryName) {
  if (!rows || rows.length === 0) {
    ...
    const prParams = context.repo({
      pull_number: prNumber,
	    ...
	    const commits = await context.octokit.rest.pulls.listCommits(prParams);
    var allAuthors = [];
    ...
    var linkOppiaAndroid = 'here'.link(
      'https://github.com/oppia/oppia-android/wiki#onboarding-instructions'
    );

    var commentBody = '';
    if (repositoryName === 'oppia-android') {
      commentBody = 'Hi! ' + userNamesNotSignedCLA +
...
module.exports.checkClaSheet = async function (auth) {
  ...
  await sheets.spreadsheets.values.get(
    ...
    async function (err, response) {
      ...
      var repositoryName = context.payload.repository.name;
