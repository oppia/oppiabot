...
var OnBoardingSheetId = process.env.ONBOARDING_SHEET_ID;
...
module.exports.generateOutput = async function (rowsCla, rowsOnBoarding,
  ...
  if (!rowsCla || rowsCla.length === 0) {
    ...
    for (var i = 0; i < allAuthors.length; i++) {
      ...
      var hasUserinOnBoardingSheet = false;
...
module.exports.checkClaStatus = async function (contextData) {
  ...
  const oauthClientWIthOnBoardingSecret = module.exports.
    authorize(JSON.parse(OnBoardingSheetId));

  // Call the sheets API with the authorized client.
  module.exports.checkClaSheet(oauthClient);
  module.exports.
    checkClaAndOnboardingTracker(oauthClient, oauthClientWIthOnBoardingSecret);
};
