...
const JOBS_AND_FEATURES_TESTING_WIKI_LINK = (
...
const mergePermissions = ['admin', 'write'];
  ...
  do {
    const fileListParams = context.repo({
      ...
      page: currentPage,
	    ...
	    const fileResponse = await context.octokit.rest.pulls.listFiles(fileListParams);
...
const checkPrIsStale = async (pullRequest, context) => {
	  const {data: lastCommit} = await context.octokit.rest.repos.getCommit(
    context.repo({
...
const getAllOpenPullRequests = async (context) => {
	  const pullRequestsResponse = await context.octokit.rest.pulls.list(
    context.repo({
      per_page: 60,
      state: 'open',
    })
...
const isUserAMemberOfTheOrganisation = async (context, username) => {
  try {
    const orgName = 'oppia';
	  const membershipCheckResponse = await context.octokit.rest.orgs.checkMembership({
      org: orgName,
      username: username,
    });
...
const isUserCollaborator = async (context, username) => {
  try {
	  const collaboratorResponse = await context.octokit.rest.repos.checkCollaborator({
...
/**
 * Check if the user has merge permissions.
 *
 * @param {import('probot').Context} context
 * @param {string} username
 */

const hasMergePrivilege = async (context, username) => {
  try {
	  const collaboratorPrivilege = await context.octokit.rest.repos
      .getCollaboratorPermissionLevel({
        owner: context.repo().owner,
        repo: context.repo().repo,
        username: username
      });
    return mergePermissions.includes(collaboratorPrivilege.data.permission);
  } catch (error) {
    // The Github API returns a 404 response if the user is not a
    // collaborator and probot throws an error for 404 responses.
    return false;
...
const getLastReviewOfSpecificType = async (
    ...
    pullRequestNumber
	  ...
	  const { data: allReviews } = await context.octokit.rest.pulls.listReviews(
    context.repo({
...
const getAllOpenIssues = async (context) => {
  ...
  do {
    // This function returns both pull requests and issues.
	    const issuesResponse = await context.octokit.rest.issues.listForRepo(
      context.repo({
...
const getAllProjectCards = async (context) => {
	  const { data: allProjects } = await context.octokit.rest.projects.listForRepo(
    context.repo()
  );
  ...
  await (async function () {
    for (let i = 0; i < allProjects.length; i++) {
      ...
      const {
        data: projectColumns,
	      } = await context.octokit.rest.projects.listColumns({
      ...
      await (async function () {
        for (let j = 0; j < projectColumns.length; j++) {
          ...
          // to get cards in a project is through it's column.
	          const { data: allCards } = await context.octokit.rest.projects.listCards({
