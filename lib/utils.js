...
const OPPIA_CODE_OWNERS_FILE_URL =
...
const hasMergePrivilege = async (context, username) => {
  try {
    const collaboratorPrivilege = await context.github.repos
 ...
 */
const getMainCodeOwnerFile = async (repository) => {
  if (repository.toLowerCase() === 'oppia-android') {
    const response = await Axios.get(OPPIA_ANDROID_CODE_OWNERS_FILE_URL);
    return response.data;
  }

  const response = await Axios.get(OPPIA_CODE_OWNERS_FILE_URL);
  return response.data;
};
