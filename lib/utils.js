...
const hasMergePrivilege = async (context, username) => {
  try {
    const collaboratorPrivilege = await context.github.repos.getCollaboratorPermissionLevel({
      owner: context.repo().owner,
      repo: context.repo().repo,
      username: username
    });
    return (collaboratorPrivilege.data.permission === 'admin') || (collaboratorPrivilege.data.permission === 'write');
  } catch (error) {
    // The Github API returns a 404 response if the user is not a
    // collaborator and probot throws an error for 404 responses.
    return false;
