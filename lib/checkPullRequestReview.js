...
const MAX_REVIEW_PERIOD = 24 * 60 * 60 * 1000;
  ...
  if (allLabels.includes(LGTM_LABEL)) {
    ...
    await context.github.issues.createComment(
      context.repo({
        ...
        ', the LGTM Label has been removed because the changes were' +
        ' requested on this PR. Thanks!.',
      })
    );
  }
};
 ...
 * @param {import('probot').Octokit.PullsGetResponse} pullRequest
 *
 * This is a function to check if there are more than 24 Hours for PR review.
 */


const isMaxReviewPeriodExceeded = async (review) => {
  let timeOfReviewSubmission = review.submitted_at;
  timeOfReviewSubmission = new Date(timeOfReviewSubmission);
  let currentTime = new Date();
  let diff = Math.abs(timeOfReviewSubmission - currentTime);
  return diff > MAX_REVIEW_PERIOD;
};

/**
 * This function handles the case for reviewers who have not reviewed PR
 * for more than 24 hrs.
 * Then this function pings those reviewers who satisfy this situtation.
 *
 * @param {import('probot').Context} context
 */
const checkForReviewersWithPendingReview = async (context) => {
  const allOpenPullRequests = await getAllOpenPullRequests(context);
  await allOpenPullRequests.map(
    async (pullRequest) => {
      ...
      const reviewersToPing = [];
      allReviews.map((review) => {
        const isReviewPeriodExceeded = isMaxReviewPeriodExceeded(review);
        // If Max Period Exceeded then We need to ping that reviewer.
        if (isReviewPeriodExceeded) {
          const reviewer = review.user.login;
          if (!reviewersToPing.includes(reviewer)) {
            reviewersToPing.push(reviewer);
          }
        }
      });

      if (reviewersToPing.length !== 0) {
        const message = 'Hi' + reviewersToPing.map((reviewer) => {
          return ' @' + reviewer.login + ' ';
        }) + ', Its been more than 24 hours, ' +
         'since the last review was performed by you.' +
...
module.exports = {
  ...
  checkForReviewersWithPendingReview
};
