...
const handleChangesRequested = async (context, pullRequest, review) => {
  console.log('Inside the function handleChangesRequested');
  ...
  if (allLabels.includes(LGTM_LABEL)) {
    ...
    await context.github.issues.createComment(
      context.repo({
        ...
        ', the LGTM Label was removed because the changes are' +
...
const handlePullRequestReview = async (context) => {
   ...
   */
  console.log('At least reached handlePullRequestReview');
