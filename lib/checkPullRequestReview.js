...
const isMaxReviewPeriodExceeded = (pullRequest) =>{
  let timeOfLastUpdate = pullRequest.updated_at;
  timeOfLastUpdate = new Date(timeOfLastUpdate);
  let currentTime = new Date.now();
  let diff = Math.abs(timeOfLastUpdate - currentTime);
...
const checkForReviewersWithPendingReview = async(context) =>{
  ...
  const requiredPrs = await allOpenPullRequests.filter(
    (pullRequest) =>{
      return hasPendingReviews(pullRequest) &&
       isMaxReviewPeriodExceeded(pullRequest);
    }
  );
