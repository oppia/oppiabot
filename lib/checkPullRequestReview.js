  ...
  await allOpenPullRequests.map(
    async (pullRequest) => {
      ...
      allReviews.map((review) => {
        const booleanVariable = isMaxReviewPeriodExceeded(review);
        // If Max Period Exceeded then We need to ping that reviewer.
        if (booleanVariable) {
          const reviewer = review.user.login;
          if (!allReviewers.includes(reviewer)) {
      ...
      if (allReviewers.length !== 0) {
        const message = 'Hi' + allReviewers.map((reviewer) => {
