...
const checkForNewCodeowner = async (context) => {
  ...
  if (changedCodeOwnerFile) {
    ...
    if (newCodeOwners.length > 0) {
      ...
      let commentBody = '';
      // Ping the codeowner of CODEOWNERS File since we just want to flag that a
      // new code-owner is added and bring it to his attention.
      commentBody =
          'Hi @' +
          userWhitelist.codeOwnerReviewer +
          ', this PR adds ' +
          codeOwnerString +
          codeOwnerFileString +
