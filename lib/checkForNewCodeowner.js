...
const getNewCodeOwners = (file) => {
  ...
  const newAdditions = changesArray.filter((change) => {
    return change.startsWith(newAddition) &&
      !change.startsWith(newCommentAddition);
  });
  ...
  if (changedCodeOwnerFile) {
    ...
    if (newCodeOwners.length > 0) {
      ...
      if (changelogLabel !== undefined) {
        // Ping project owner if the changelog label is present.
        ...
        // Ping one of the reviewers since we just want to flag that a
        // new code-owner is added and bring it to the attention of any
