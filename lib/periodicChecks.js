...
/**
 * This function comments on and tags pull requests in which the last commit was created 2 days ago
 * @param {import('probot').Context} context
 */
const checkAndTagPRsWithOldBuilds = async (context) => {
  const OLD_BUILD_LABEL = "PR: don't merge - STALE BUILD";
  console.log({MIN_BUILD_DATE: utilityModule.MIN_BUILD_DATE});
  ...
  allOpenPullRequests.forEach(async (pullRequest) => {
    ...
    const lastCommitSha = pullRequest.head.sha;

    const pRCommits = await context.github.pulls.listCommits(
      context.repo({ pull_number: pullRequest.number })
    );

    const lastCommit = pRCommits.data.find(
