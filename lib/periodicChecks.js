...
/**
 * 1. Check if the new issue have a project assigned.
 * 2. If not then it will ping Core Maintainers
 * @param {import('probot').Context} context
 */
const ensureNewIssuesHaveProjects = async(context) =>{
  // This sleep function is to provide user some time to add project after
  // making a issue, If this is not added then oppiabot will comment on the
  // issue to add project as soon as issue is formed.
  await sleep(20000);
  const issue = context.payload.issue;
  const allProjectCards = await getAllProjectCards(context);

  const doesIssueHaveProject = allProjectCards.some((card) => {
    return card.content_url.endsWith(issue.number);
  });

  if (!doesIssueHaveProject) {
    context.github.issues.createComment(
      context.repo({
        issue_number: issue.number,
        body:
          ...
          oppiaMaintainers +
          ', this issue is not assigned to ' +
          'any project. Can you please assign it to the some project? Thanks!',
      })
    );
  }
};
