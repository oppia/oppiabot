...
const ensureAllIssuesHaveProjects = async (context) => {
  ...
  const issuesWithoutProjects = allOpenIssues.filter((issue) => {
    const issueBelongsToProject = allProjectCards.some((card) => {
      return card.content_url && card.content_url.endsWith(issue.number);
    });
...
/**
 * 1. Check if the new issue has a project assigned.
 * 2. If not then then ping the Core Maintainers.
 * @param {import('probot').Context} context
 */
const ensureNewIssuesHaveProjects = async(context) =>{
  // This sleep function is used to provide the user some
  // time to add project after making the issue.
  await sleep(20000);
