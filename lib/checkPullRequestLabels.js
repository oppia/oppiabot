...
module.exports.checkChangelogLabel = async function (context) {
  ...
  const pullRequestNumber = pullRequest.number;
  const TEN_SECONDS_IN_MSECS = 10 * 1000;

  await utilityModule.sleep(TEN_SECONDS_IN_MSECS);
  ...
  if (label.name === HOTFIX_LABEL) {
    var commentParams = context.issue({
        body:
          ...
          'Please ensure that you add the \'PR: for current release\' label '
          'if the next release is in progress. Thanks!'
      });
    await context.github.issues.createComment(commentParams);
  }
};
