 ...
 * @param {import('probot').Context} context
 */
module.exports.checkForMilestone = async function (context) {
  /**
   * @type {import('probot').Octokit.PullsGetResponse} context
   */
  const milestone = context.payload.pull_request.milestone;
  const repoMilestones = await context.github.issues.listMilestonesForRepo({
    owner: context.repo.owner,
    repo: context.repo.repo
  });
  if (repoMilestones.includes(milestone.title)) {
    const user = context.payload.sender;
    const commentBody =
    ...
    await context.github.issues.createComment(commentParams);
    await context.github.issues.update({milestone: null});
  }
};
