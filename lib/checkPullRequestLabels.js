...
module.exports.checkChangelogLabel = async function (context) {
  ...
  const hasChangesBeingRequested = doesPullRequestHaveChangesRequested(
  ...
  if (hasChangelogLabel) {
    ...
    else if (!hasChangesBeingRequested) {
      const author = pullRequest.user.login;
      const authorIsAssigned = pullRequest.assignees.some(assignee => assignee.login === author)
      if (authorIsAssigned) {
        ...
        await context.github.issues.createComment(
          context.repo({
            issue_number: pullRequest.number,
            body: 'Unassigning @' + author + ' since the changelog label was added.',
          })
        );
      }
    }
