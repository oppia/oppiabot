const CRITICAL_LABEL_NAME = 'critical';
const { releaseCoordinators, SERVER_JOBS_ADMIN } = require('../userWhitelist.json');
...
module.exports.checkCriticalLabel = async function(context) {
  ...
  if(label.name === CRITICAL_LABEL_NAME) {
    ...
    if (isUserAllowedToRemoveCriticalLabel(user.login)) {
      // Add label back with an interesting message
      const addLabelParams = context.issue({
        labels: [CRITICAL_LABEL_NAME],
      });
...
module.exports.checkForIssueLabel = async function(context) {
  ...
  if (!label.name.startsWith('PR') && !prLabels.includes(label.name)) {
    ...
    const commentBody =
      ...
      'used on issues. I’m removing the label. You can learn more about ' +
      'labels ' + link + '. Thanks!';
