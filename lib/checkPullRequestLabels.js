...
module.exports.checkAssignee = async function (context) {
  ...
  const changelogLabel = context.payload.label.name.trim();
  const hasChangelogLabel = isChangelogLabel(changelogLabel);
...
module.exports.checkChangelogLabel = async function (context) {
  ...
  const hasChangesRequested = utilityModule.doesPullRequestHaveChangesRequested(
  ...
  if (hasChangelogLabel) {
    ...
    if (!changelogLabelIsValid) {
      ...
      const failedCommentParams = context.issue({
        body:
      ...
      context.pullRequest.reviewer = [{login:'srijanv9'}];
    } else if (!hasChangesRequested) {
      const author = pullRequest.user.login;
      const authorIsAssigned = pullRequest.assignees.some(
        assignee => assignee.login === author);
      if (authorIsAssigned) {
        // Unassign the author as the changelog label is added
        ...
        await context.github.issues.createComment(
          context.repo({
            issue_number: pullRequest.number,
            body: 'Unassigning @' + author +
            ' since the changelog label was added.',
          })
        );
      }
    }
  ...
  const isCollaborator = await isUserCollaborator(context, userName);
  if (!isCollaborator) {
    ...
    if (reviewers.length === 1) {
      ...
      // There are no reviewers for this PR. Usually due to the PR being
      // in draft mode.
      console.log({pullRequest});
      if (pullRequest.draft) {
        // Do nothing since no review is required here.
        return;
      } else {
        // No reviewers in the PR due to some other reason.
        // Ping the welfare team lead
        commentBody =
          'Hi @' + teamLeads.welfareTeam +
          ', could you please add the appropriate changelog label to this ' +
          'pull request? Thanks!';
        reviewersToPing = [teamLeads.welfareTeam];
      }
    }
