...
const compare = (CRON_REGISTRY_FILENAME, changedFiles) => {
  CRON_REGISTRY_FILENAME.forEach((e1) => changedFiles.forEach((e2) => {
    if(e1 === e2) {
      return true;
    }
  }));
...
const getCronCommentBody = (changedFiles, prAuthor) => {
  const newLineFeed = '<br>';
  const cronJobsForm = 'cron jobs form'.link('');// some link
  const wikiLinkText = (
    'this guide'.link(
