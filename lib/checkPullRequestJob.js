...
const CRON_REGISTRY_FILENAME = ''; // this requires some filename
const CRON_TEST_DIR_PREFIX = 'core/'; // this is also not fixed yet
const CRON_JOB_REGEX = new RegExp(
  [
    '(?<addition>\\+)(?<classDefinition>class\\s)',
    '(?<name>[a-zA-Z]{2,256})(?<suffix>OneOffJob)(?<funDef>\\()'
  ].join('')
 ...
 */
const isInCronDir = (filename) => {
  return filename.startsWith(CRON_TEST_DIR_PREFIX);
};

/**
 * @param {import('probot').Octokit.PullsListFilesResponseItem} file
 */
const getNewCronJobsFromFile = (file) => {
  return getNewItemsFromFileByRegex(CRON_JOB_REGEX, file);
};

/**
 * @param {import('probot').Octokit.PullsListFilesResponseItem} file
 */
const addsNewCronJob = (file) => {
  const newJobs = getNewCronJobsFromFile(file);
  return newJobs.length > 0;
};

/**
 * Returns the name of the cron job module from it's file name.
 * @param {string} filename
 *
 * @example
 * getCronJobModuleName('core/domain/user_jobs_one_off.py')
 * // returns user_jobs_one_off
 */
const getCronJobModuleName = (filename) => {
 ...
 */
const isCronRegistryUpdated = (job, files) => {
  const registryFile = files.find(({ filename }) => {
    return filename.includes(CRON_REGISTRY_FILENAME);
  });
 ...
 */
const getCronRegistryReminder = (newJobFiles, changedFiles) => {
  let registryReminder = ', ';

  const hasNotAddedFileToRegistry = newJobFiles.some(
    (file) => {
      const jobs = getNewJobsFromFile(file);
      const jobModuleName = getCronJobModuleName(file.filename);
      return jobs.some((job) => !isCronRegistryUpdated(
 ...
 * @param {import('probot').Octokit.PullsListFilesResponseItem[]} newJobFiles
 */
const getCronJobNameString = (newJobFiles) => {
...
const getCronCommentBody = (newJobFiles, changedFiles, prAuthor) => {
  const registryReminder = getCronRegistryReminder(newJobFiles, changedFiles);
  const jobNameString = getCronJobNameString(newJobFiles);
  const newLineFeed = '<br>';
  const cronJobsForm = 'cron jobs form'.link(
    '');// some link
  const wikiLinkText = (
    'this guide'.link(
  ...
  if (newJobFiles.length === 1) {
    const serverAdminPing = (
      ...
      ', PTAL at this PR, ' + 'it adds a new cron job.' + jobNameString
    );

    const prAuthorPing = 'Also @' + prAuthor + registryReminder +
      'please make sure to fill in the ' + cronJobsForm +
    ...
    const serverAdminPing = (
      'Hi @' + serverJobAdmins.join(', @') +
      ', PTAL at this PR, ' + 'it adds new cron job.' + jobNameString);
...
const checkForNewCronJob = async (context) => {
  ...
  if (!hasDatastoreLabel(pullRequest)) {
    ...
    const newJobFiles = changedFiles.filter((file) => {
      return !isInCronDir(file.filename) && addsNewCronJob(file);
    });

    if (newJobFiles.length > 0) {
      const commentBody = getCronCommentBody(
        newJobFiles,
        changedFiles,
